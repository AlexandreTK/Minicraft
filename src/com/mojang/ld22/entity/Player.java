...
public class Player extends Mob {
	...
	public int staminaRecharge = 0; // time to reload the stamina
	public int staminaRechargeDelay = 0;
	public int score = 0;
	public int maxStamina = 10; // max energy of the player
	private int onStairDelay = 0;
	...
	public void tick() {
		...
		int xa = 0;
		int ya = 0;
		if (input.up.isKeyDown())
			ya--;
		if (input.down.isKeyDown())
			ya++;
	...
	private boolean use() {
		...
		if (direction == 0 && use(positionX - 8, positionY + 4 + yo, positionX + 8, positionY + 12 + yo))
			return true;
		if (direction == 1 && use(positionX - 8, positionY - 12 + yo, positionX + 8, positionY - 4 + yo))
			return true;
		if (direction == 3 && use(positionX + 4, positionY - 8 + yo, positionX + 12, positionY + 8 + yo))
			return true;
		if (direction == 2 && use(positionX - 12, positionY - 8 + yo, positionX - 4, positionY + 8 + yo))
			return true;
	...
	private void attack() {
		...
		if (activeItem != null) {
			...
			int yo = -2;
			int range = 12;
			if (direction == 0 && interact(positionX - 8, positionY + 4 + yo, positionX + 8, positionY + range + yo))
				done = true;
			if (direction == 1 && interact(positionX - 8, positionY - range + yo, positionX + 8, positionY - 4 + yo))
				done = true;
			if (direction == 3 && interact(positionX + 4, positionY - 8 + yo, positionX + range, positionY + 8 + yo))
				done = true;
			if (direction == 2 && interact(positionX - range, positionY - 8 + yo, positionX - 4, positionY + 8 + yo))
				done = true;
			if (done)
				return;
		...
		if (activeItem == null || activeItem.canAttack()) {
			...
			int range = 20;
			if (direction == 0)
				hurt(positionX - 8, positionY + 4 + yo, positionX + 8, positionY + range + yo);
			if (direction == 1)
				hurt(positionX - 8, positionY - range + yo, positionX + 8, positionY - 4 + yo);
			if (direction == 3)
				hurt(positionX + 4, positionY - 8 + yo, positionX + range, positionY + 8 + yo);
			if (direction == 2)
				hurt(positionX - range, positionY - 8 + yo, positionX - 4, positionY + 8 + yo);
